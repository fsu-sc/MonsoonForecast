...
# for cur_year in range(start_year, start_year+3):
    ...
    for i, cur_field in enumerate(fields):
        ...
        axs[2,i].plot(range(365 - roll_size), year_cum_anomaly[i, cur_year_idx, :365 - roll_size], label='cum anomaly', color='green')
        axs[2,i].set_title(f"Cumulative anomaly {cur_field} for year {cur_year}")
        # Scatter the onset day
        axs[2,i].scatter(onset_day, year_cum_anomaly[i, cur_year_idx, onset_day], color='red', label='onset day')
# 
